...
function federexperiment(;
    ...
    samples = linspace(xmin, xmax, nx) |> collect

    ys = map(odeohnetreatment, samples)
    ...
    zs = map(x->x + rho_std*randn(2), ys)

    Ldx = [rho_error(z-d)  for d  in datas, z in ys]
    Lzx = [rho_error(z-zp) for zp in zs,   z in ys]
end
