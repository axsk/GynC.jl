...
rho(x) = g(x,0,10)
prior(x) = (1/8*(x/40)^2*(1-x/40)^2*(x>0)*(x<40)*30/40 + 7/8*g(x,78,21))
...
function priorestimation(y::Vector; args...)
  mean([posterior(yi; args...) for yi in y])
end

""" Given some measured y value, sample the posterior distribution. """
function posterior(y::Real; iters=1000, thin=1)
  m = Model(
    x = Stochastic(()->DensityDistribution(x->rho(y-h(x)))))

  #setsamplers!(m, [Slice([:x],[100.0])])
	setsamplers!(m, [NUTS([:x])])
  data =  Dict{Symbol,Any}()
  init = [Dict{Symbol,Any}(:x => 60) for i=1:3]

  mcmc(m, data, init, iters, burnin=0, thin=thin, chains=3)
end
